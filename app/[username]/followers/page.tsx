...
export default async function FollowersPage({ params }: FollowersPageProps) {
  ...
  const followingIds = new Set(user.following.map((follow) => follow.followingId));
    ...
    <main className="px-6 py-10 text-white">
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-8">
        <Header
          ...
          title="Seguidores"
          detail="Personas que siguen tu perfil."
        />
        ...
        {user.followers.length > 0 ? (
          ...
          <EmptyState title="Aun no tienes seguidores." />
        )}
      </section>
    </main>
  );
  ...
  return (
    <div className="rounded-lg border border-dashed border-zinc-800 bg-zinc-950/50 p-8">
      ...
      <p className="mt-2 text-zinc-400">
        Cuando haya actividad social en tu perfil, aparecera aqui.
      </p>
    </div>
  );
}
